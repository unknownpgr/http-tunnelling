...
fn send(mut stream: &TcpStream, t: u8, id: u32, data: Vec<u8>) -> Result<usize> {
    let mut frame = vec![t];
...
pub fn register(stream: &TcpStream, data: Vec<u8>) -> Result<usize> {
    send(&stream, FRAME_TYPE_REGISTER, 0, data)
}

pub fn send_data(stream: &TcpStream, id: u32, data: Vec<u8>) -> Result<usize> {
    send(&stream, FRAME_TYPE_DATA, id, data)
}

pub fn send_log(stream: &TcpStream, data: Vec<u8>) -> Result<usize> {
    send(&stream, FRAME_TYPE_LOG, 0, data)
}

pub fn send_close(stream: &TcpStream, id: u32) -> Result<usize> {
    let data = vec![];
    send(&stream, FRAME_TYPE_CLOSE, id, data)
}

pub fn send_heartbeat(stream: &TcpStream) -> Result<usize> {
    let data = vec![];
    send(&stream, FRAME_TYPE_HEARTBEAT, 0, data)
}

pub fn send_heartbeat_ack(stream: &TcpStream) -> Result<usize> {
    let data = vec![];
